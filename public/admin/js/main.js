    ...
    post.forEach((post) => {
        let postHTML = `
                                    <article class="d-flex justify-content-between align-items-center article-inline">
                                        ...
                                        <div class="date w30">${post.data}</div>
...
async function addCallbackRequests() { // The next thing is to create a function for displaying the callback-request on a web page.
    ...
    requests.forEach((requests) => {
        let requestHTML = `
                                    <article class="d-flex justify-content-between align-items-center article-inline">
                                        ...
                                        <div class="date w30">${requests.data}</div>
                                        <div class="remove w5"><button class="btn btn-link btn-remove">X</button></div>
                                    </article>`;
...
async function addEmails() { // The next thing is to create a function for displaying the callback-request on a web page.
